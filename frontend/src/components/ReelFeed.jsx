...
const ReelFeed = ({ items = [], onLike, onSave, emptyMessage = 'No videos yet.' }) => {
  ...
  const resolveFoodPartnerId = (foodPartner) => {
    if (!foodPartner) return null
    // if populated object
    if (typeof foodPartner === 'object' && foodPartner._id) return foodPartner._id
    // if already an id string
    return String(foodPartner)
  }
          ...
          <section key={item._id} className="reel" role="listitem">
            ...
            <div className="reel-overlay">
              ...
              <div className="reel-actions">
                <div className="reel-action-group">
                  <button
                    onClick={onLike ? () => onLike(item._id) : undefined}
                    className="reel-action"
                    aria-label="Like"
                    type="button"
                  >
                ...
                <div className="reel-action-group">
                  <button
                    className="reel-action"
                    onClick={onSave ? () => onSave(item._id) : undefined}
                    aria-label="Bookmark"
                    type="button"
                  >
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="reel-content">
                <p className="reel-description" title={item.description}>{item.description}</p>
                {item.foodPartner && (() => {
                  const partnerId = resolveFoodPartnerId(item.foodPartner)
                  return partnerId ? (
                    <Link className="reel-btn" to={`/food-partner/${partnerId}`} aria-label="Visit store">Visit store</Link>
                  ) : null
                })()}
              </div>
            </div>
